...
latency = pd.read_csv('./put_4096_10_256.csv',usecols=['latency']).apply(pd.to_numeric).values

latency=latency*1000

plt.title('Write Latency Distribution')
plt.subplot(211)
...
plt.savefig('./write_tail_latency.png')
plt.show()
...
with open("write_tail_latency.txt", "w") as file:
    file.write("write tail latency:\n")
...
plt.grid()
plt.show()
plt.savefig('./write_tail_latency_queue.png')
